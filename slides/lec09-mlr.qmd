...
```{r}
#| echo: false
amod <- lm(FEV1 ~ AGE + HEIGHT + BIOL.SEX + AREA , data=fev_long)
```

## Assumption - Linearity {.smaller}

* The assumption is that the linear combination $XB$ is linearly associated with Y. That's difficult (impossible for higher dimensions) to graph. So we look at the linearity of the _residuals_ with respect to our fitted values. 
* A non-linear association may indicate that some of the predictors may have a non-linear relationship with the response (such as a quadratic or cubic), or that the response variable should be transformed. 
* See more at: [https://easystats.github.io/performance/articles/check_model.html#linearity](https://easystats.github.io/performance/articles/check_model.html#linearity)

## Assumption - Variance inflation {.smaller}
...
* In multiple regression, the standard error of $b_i$ depends on the correlation between $x_i$, and _all other x's_. This is quantified as $R^2_i$. 
* As this correlation increases, the _variance inflation fraction_ (VIF) increases.
* Common thresholds suggest that VIF values less than 5 indicate low collinearity, values between 5 and 10 indicate moderate collinearity, and values larger than 10 indicate high collinearity
* See more at [https://easystats.github.io/performance/articles/check_model.html#multicollinearity](https://easystats.github.io/performance/articles/check_model.html#multicollinearity) 
