...
ans <- datasets::anscombe %>% mutate(rowid = 1:n()) %>% 
  ...
  tidyr::pivot_wider(names_from = "dim", 
              values_from = "val") %>%
  mutate(dataset = paste("Anscombe Data", ds)) %>%
  filter(dataset == "Anscombe Data 1")

rawr <- datasauRus::datasaurus_dozen %>%
  filter(dataset %in% c("dino", "dots", "slant_up" )) %>% 
  bind_rows(ans)

ggplot(rawr, aes(x=x, y=y, color = dataset)) + 
  geom_point(size = 2) + 
