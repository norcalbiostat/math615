...
ggsave(filename="plot2norm.png")
```

# Quantitative Outcome ~ Binary Covariate {background-color="#6c972b" background-image="plot2norm.png" background-opacity="0.7"}

$$y_{ij} = \mu_{j} + \epsilon_i \, , \qquad x_{j} \sim \mathcal{N}(\mu_{j}, \sigma^2)$$

* Response data $y_{ij}$ from observation $i=1\ldots n$ belonging to group $j=1,2$ 
* Explanatory data $x_{ij}$ is normally distributed around a group mean $\mu_j$
