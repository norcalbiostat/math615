...
1. Create a `dm_dataname.qmd` or `dm_dataname.sps` script file. 
    - Rename this to include your username. I.e. `dm_dataname_rdonatello.qmd`. 
    - Make sure this is your **math615/script** folder, and your data is in your **math615/data** folder
2. After you load the data, restrict the variables to only the ones you are investigating using statements like `select` from the `dplyr` package, or the `KEEP` statement in SPSS. 
...
4. Save the resulting data set to your `data` folder as `datasetname_clean.Rdata`, or `datasetname_clean.sav`. e.g. `addhealth_clean.Rdata` using the `save()` function in R, or `SAVE OUTFILE` command in SPSS. 
...
## Rmarkdown
Here is an example using R Markdown/Quarto
:::
