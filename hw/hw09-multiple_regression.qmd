...
1. For all analyses in this assignment, you are to display the raw output (e.g. the results of `lm` directly) *AND* using a nice table output using `tbl_regression`. 
2. Show me the coding for the binary response variable used in logistic regression. 
:::
...
ggpubr::ggdensity(data.for.model4, x = "sleep_duration", 
                  fill = "poverty_cat", facet.by = "female_c")
```
